...
# Bacillus sp. plot
## Sodosol
```{r bacillusblot2, fig.cap = "Total read counts, relative abundances and clr transformed abundances of bacillus in Sodosol amended without (control) and with NPKS, poultry litter (PL), poultry litter/N and straw/NPKS. Significance to NPKS is shown (Wilcoxon test)." , echo=FALSE, fig.width=8, fig.asp = 0.45, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE, include=FALSE}
#barplot
...
bacilluscounts <- subs  %>% ggbarplot(data = ., x = "Treatments", y = "Abundance", add = "mean_se" , fill = "Treatments", facet.by = "Genus", width = 0.5, xlab = "", position = position_dodge(0.7), scales = "free", ylab = "Total read counts" ) +  stat_compare_means(method = "wilcox", ref.group = "NPKS",label = "p.signif")+ theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
bacillusrelative <- subs  %>% ggbarplot(data = ., x = "Treatments", y = "Abundance", add = "mean_se" , fill = "Treatments", facet.by = "Genus", width = 0.5, xlab = "", position = position_dodge(0.7), scales = "free", ylab = "Relative abundances") +  stat_compare_means(method = "wilcox", ref.group = "NPKS",label = "p.signif")+ theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
bacillusclr <- subs  %>% ggbarplot(data = ., x = "Treatments", y = "Abundance", add = "mean_se" , fill = "Treatments", facet.by = "Genus", width = 0.5, xlab = "", position = position_dodge(0.7), scales = "free", ylab = "Abundances clr transformed") +  stat_compare_means(method = "wilcox", ref.group = "NPKS",label = "p.signif")+ theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
## Vertisol
```{r bacillusblot3, fig.cap = "Total read counts, relative abundances and clr transformed abundances of bacillus in Vertisol amended without (control) and with NPKS and poultry litter/N. Significance to NPKS is shown (Wilcoxon test).", echo=FALSE, fig.width=8, fig.asp = 0.45, message=FALSE, warning=FALSE, cache=TRUE}
#barplot
...
physeqB.flt2 <- prune_samples(sample_names(physeqB) != "Juan07", physeqB)
physeqB.flt2 <- prune_samples(sample_data(physeqB.flt2)$Soiltypes == "Sodosol", physeqB.flt2)
physeqB.flt2 <- prune_samples(sample_data(physeqB.flt2)$Treatments %in% c("NPKS","PL/N"), physeqB.flt2)
physeqB.flt2 = prune_taxa(taxa_sums(physeqB.flt2) != 0, physeqB.flt2)
physeqB.flt2 = prune_taxa(taxa_sums(physeqB.flt2) >= 10, physeqB.flt2) #minimum reads per feature
#agglomerate to genus level
physeqB.flt2 <- tax_glom(physeqB.flt2 , "Genus")
...
physeqB.flt2 <- prune_samples(sample_names(physeqB) != "Juan07", physeqB)
physeqB.flt2 <- prune_samples(sample_data(physeqB.flt2)$Treatments %in% c( "NPKS","PL/N", "PL","Straw+NPKS"), physeqB.flt2)
physeqB.flt2 = prune_taxa(taxa_sums(physeqB.flt2) != 0, physeqB.flt2)
physeqB.flt2 = prune_taxa(taxa_sums(physeqB.flt2) >= 10, physeqB.flt2) #minimum reads per feature
#agglomerate to genus level
physeqB.flt2 <- tax_glom(physeqB.flt2 , "Genus")
...
```{r aldex_pathways, eval=TRUE, echo=FALSE, fig.width=15, fig.asp = 1.7, message=FALSE, warning=FALSE, cache=TRUE}
# Aldex

#Sodosol NPK vs PL 
require(ALDEx2)
physeqpw.flt <- prune_samples(sample_data(physeq.pw)$Soiltypes == "Sodosol", physeq.pw)
physeqpw.flt <- prune_samples(sample_data(physeqpw.flt)$Treatments %in% c("NPKS","PL"), physeqpw.flt)
...
require(ALDEx2)
physeqpw.flt <- prune_samples(sample_data(physeq.pw)$Soiltypes == "Sodosol", physeq.pw)
physeqpw.flt <- prune_samples(sample_data(physeqpw.flt)$Treatments %in% c("NPKS","Straw+NPKS"), physeqpw.flt)
physeqpw.flt = prune_taxa(taxa_sums(physeqpw.flt) != 0, physeqpw.flt)
physeqpw.flt = prune_taxa(taxa_sums(physeqpw.flt) >= 10, physeqpw.flt) #minimum reads per feature
physeqpw.flt = filter_taxa(physeqpw.flt , function(x) sum(x > 0) > (0.17*length(x)), TRUE) 
taxtable.flt2 <- tax_table(physeqpw.flt)@.Data %>% as.data.frame() %>% rownames_to_column("Id")
...
require(ALDEx2)
physeqpw.flt <- prune_samples(sample_data(physeq.pw)$Soiltypes == "Sodosol", physeq.pw)
physeqpw.flt <- prune_samples(sample_data(physeqpw.flt)$Treatments %in% c("NPKS","PL/N"), physeqpw.flt)
physeqpw.flt = prune_taxa(taxa_sums(physeqpw.flt) != 0, physeqpw.flt)
physeqpw.flt = prune_taxa(taxa_sums(physeqpw.flt) >= 10, physeqpw.flt) #minimum reads per feature
physeqpw.flt = filter_taxa(physeqpw.flt , function(x) sum(x > 0) > (0.17*length(x)), TRUE) 
taxtable.flt2 <- tax_table(physeqpw.flt)@.Data %>% as.data.frame() %>% rownames_to_column("Id")
...
aldex_significant_features_presenttwice <- aldex_significant_features %>% 
  group_by(Id) %>% 
  filter(n()>1) %>% filter(we.eBH <= 0.0002)

## plot the genera whose abundances were significantly different in at least two of the treatments vs NPKS

physeqpw.flt <- prune_samples(sample_data(physeq.pw)$Treatments %in% c( "NPKS","PL/N", "PL","Straw+NPKS"), physeq.pw)
physeqpw.flt = prune_taxa(taxa_sums(physeqpw.flt) != 0, physeqpw.flt)
physeqpw.flt = prune_taxa(taxa_sums(physeqpw.flt) >= 10, physeqpw.flt) #minimum reads per feature
#agglomerate to genus level
...
subs  %>% ggbarplot(data = ., x = "Treatments", y = "Abundance", add = "mean_se", facet.by = c("OTU") , fill = "CLASS1", width = 0.5, xlab = "", position = position_dodge(0.7), scales = "free", ylab = "Relative abundances", ncol = 3) + ggtitle("Pathways different to NPKS in at least two other treatments (Aldex p < 0.05)") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
