...
```{r inspec, warning=FALSE, message=FALSE, echo=FALSE, cache = FALSE}
# all features
...
## NMDS Bray
### Sodosol (n = 36) and Vertisol (n = 36)
...
nmds2 <- phyloseq::plot_ordination(ps.perm, ord_nmds , color = "Treatments", shape = "Treatments")  + theme( panel.grid.major = element_blank(), panel.grid.minor = element_blank() ) + xlim(xaxis) + ylim(yaxis) + geom_hline(yintercept=0, linetype="dashed", color = "gray") + geom_vline(xintercept=0, linetype="dashed", color = "gray") + annotate(geom = 'text', label = paste("Bray-Curtis, Stress = ", label), x = -Inf, y = -Inf, hjust = -0.05, vjust = -1, size = 3) + annotate(geom = 'text', label = paste("Vertisol"), x = -Inf, y = 0.6, hjust = -0.2, vjust = 0, size = 4)
#+ ggtitle("nMDS, bray") + labs(subtitle ="filtered and rarefied OTU table")
```

```{r nmdsplot, fig.width= 6, fig.asp=1.15, message=FALSE, warning=FALSE, echo=FALSE, results=FALSE, cache = FALSE,  fig.cap = "Beta diversity comparisons by non-metric multidimensional scaling (NMDS) among different amendment treatments in the Sodosol (a) and Vertosol (b) subsoils."}
ggpubr::ggarrange(nmds1, nmds2, ncol = 1, labels = "auto") 
```

go [back to top](#metadata) 

### Sodosol (n = 18) and Vertisol (n = 18), Aggregate size >2 mm only
...
```{r nmdsplot2mm, fig.width= 6, fig.asp=1.15, message=FALSE, warning=FALSE, echo=FALSE, results=FALSE, cache = FALSE,  fig.cap = "Beta diversity comparisons by non-metric multidimensional scaling (NMDS) among different amendment treatments in the Sodosol (a) and Vertosol (b) subsoils."}
ggpubr::ggarrange(nmds1, nmds2, ncol = 1, labels = "auto") 
```

go [back to top](#metadata) 

### Sodosol (n = 18) and Vertisol (n = 18), Aggregate size 0.25-2 mm only
