...
# Bacillus sp. plot
## Both soils together
```{r bacillusblot, echo=FALSE, fig.width=8, fig.asp = 0.35, message=FALSE, warning=FALSE, cache=TRUE}
#barplot
...
## Sodosol only
```{r bacillusblot2, echo=FALSE, fig.width=8, fig.asp = 0.35, message=FALSE, warning=FALSE, cache=TRUE}
#barplot
...
## Vertisol only
```{r bacillusblot3, echo=FALSE, fig.width=8, fig.asp = 0.35, message=FALSE, warning=FALSE, cache=TRUE}
#barplot
...
subs <- physeqB.flt2.mlt.count %>% dplyr::filter(Genus %in% c("Bacillus")) 
bacilluscounts <- subs  %>% ggbarplot(data = ., x = "Genus", y = "Abundance", add = "mean_se" , fill = "Treatments", xlab = "", scales = "free", ylab = "Total read counts", width = 0.5, position = position_dodge(0.7) )  + scale_fill_manual(values = c("tomato","khaki4","mediumseagreen") )
...
subs <- physeqB.flt2.mlt.rel %>% dplyr::filter(Genus %in% c("Bacillus")) 
bacillusrelative <- subs  %>% ggbarplot(data = ., x = "Genus", y = "Abundance", add = "mean_se", fill = "Treatments", xlab = "", scales = "free", ylab = "Relative abundances", width = 0.5, position = position_dodge(0.7))  + scale_fill_manual(values = c("tomato","khaki4","mediumseagreen") )
...
subs <- physeqB.flt2.mlt.clr %>% dplyr::filter(Genus %in% c("Bacillus")) 
bacillusclr <- subs  %>% ggbarplot(data = ., x = "Genus", y = "Abundance", add = "mean_se", fill = "Treatments", xlab = "", scales = "free", ylab = "Abundances clr transformed", width = 0.5, position = position_dodge(0.7))  + scale_fill_manual(values = c("tomato","khaki4","mediumseagreen") )
...
## Sodosol NPK vs PL 
```{r aldex1, eval=TRUE, echo=FALSE, fig.asp=0.8, fig.width=9, message=FALSE, warning=FALSE, cache=TRUE}
# Aldex
...
physeqB.flt2.sod <- tax_glom(physeqB.flt2 , "Genus")
taxtable.flt2 <- tax_table(physeqB.flt2.sod)@.Data %>% as.data.frame() %>% rownames_to_column("Id")

summary(sample_data(physeqB.flt2.sod)$Treatment)
...
physeqB.flt2.clr.sod <- microbiome::transform(physeqB.flt2.sod, "compositional")
physeqB.flt2.mlt.clr.sod <- psmelt(physeqB.flt2.clr.sod)

subs <- physeqB.flt2.mlt.clr.sod %>% dplyr::filter(Genus %in% as.vector(aldex_significant_features$Genus) )
